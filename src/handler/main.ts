...
import { commands, slashes } from '../index';
...
export default async (client: Client) => {
  try {
    ...
    for (const folders of commandFiles) {
      const folder = await readdir(join(__dirname, `../commands/cmd/${folders}`));
    ...
    for (const folders of slashFiles) {
      const folder = await readdir(join(__dirname, `../commands/slash/${folders}`));
    ...
    for (const file of eventFiles) {
      ...
      client.on(e.default.event, (...args) => e.default.listener(client, ...args));
    }
  } catch (err) {
    console.error(err);
  }
